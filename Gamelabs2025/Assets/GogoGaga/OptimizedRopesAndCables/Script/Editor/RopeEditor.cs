...
namespace GogoGaga.OptimizedRopesAndCables
{
    ...
    public class RopeEditor : Editor
    {
        ...
        // NavMesh collision properties
        private SerializedProperty useNavMeshCollision;
        private SerializedProperty navMeshSampleCount;
        private SerializedProperty navMeshRaycastDistance;
        private SerializedProperty navMeshOffset;
        ...
        private void OnEnable()
        {
            ...
            // Initialize NavMesh properties
            useNavMeshCollision = serializedObject.FindProperty(nameof(Rope.useNavMeshCollision));
            navMeshSampleCount = serializedObject.FindProperty(nameof(Rope.navMeshSampleCount));
            navMeshRaycastDistance = serializedObject.FindProperty(nameof(Rope.navMeshRaycastDistance));
            navMeshOffset = serializedObject.FindProperty(nameof(Rope.navMeshOffset));
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            RopeProperties();
            RopeCurve();
            NavMeshCollisionSettings(); // Added NavMesh section
        ...
        // New method for NavMesh settings
        private void NavMeshCollisionSettings()
        {
            EditorGUILayout.Space(10);
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.LabelField("NAVMESH COLLISION", EditorStyles.boldLabel);
            EditorGUILayout.Space(2);

            EditorGUILayout.PropertyField(useNavMeshCollision, new GUIContent("Use NavMesh Collision", "Should the rope collide with and follow the NavMesh surface?"));
            
            // Only show additional NavMesh settings if collision is enabled
            if (useNavMeshCollision.boolValue)
            {
                EditorGUI.indentLevel++;
                
                EditorGUILayout.PropertyField(navMeshSampleCount, new GUIContent("Sample Count", "How many segments to check for NavMesh collision"));
                
                EditorGUILayout.PropertyField(navMeshRaycastDistance, new GUIContent("Raycast Distance", "How far to check downward for the NavMesh"));
                component.navMeshRaycastDistance = component.navMeshRaycastDistance < 0.1f ? 0.1f : component.navMeshRaycastDistance;
                
                EditorGUILayout.PropertyField(navMeshOffset, new GUIContent("Surface Offset", "Height offset from the NavMesh surface"));
